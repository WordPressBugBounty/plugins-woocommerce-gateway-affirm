...
class WooCommerce_Gateway_Affirm {
	...
	public function possibly_add_capture_charge_bulk_order_action() {
		...
		if ( 'shop_order' === $this->get_post_type() && 'trash' !== $post_status ) {
			wp_enqueue_script(
				'woocommerce-affirm-inline-admin-order',
				plugins_url( 'assets/js/affirm-inline-admin-order.js', __FILE__ ),
				array( 'jquery' ),
	...
	public function custom_bulk_admin_notices() {
		...
		if ( 'edit.php' === $pagenow
			...
			$capture_count = absint( wc_clean( wp_unslash( $_REQUEST['captured'] ) ) );
	...
	protected function render_affirm_monthly_payment_messaging(
		...
		if ( ( ( $amount > $this->get_gateway()->min * 100 )
			...
			|| 'product' === $attrs['page-type']
			|| 'category' === $attrs['page-type']
		) {
			echo '<p id="learn-more" class="affirm-as-low-as"';
			foreach ( $attrs as $attr => $val ) {
				if ( ! $val ) {
					continue;
				}
				printf( ' data-%s="%s"', esc_attr( $attr ), esc_attr( $val ) );
			}
			echo '>-</p>';
		}
	}
	...
	public function display_order_fee( $order_id ) {
		...
		<tr>
			...
			<td width="1%"></td>
			<td class="total">
				-&nbsp;<?php esc_attr(0.01 * $this->get_gateway()->getOrderMeta( $order_id, 'fee_amount' )); ?>
			</td>
		</tr>
			<?php
	}
		...
		if ( $this->get_gateway()->inline ) {
			...
			wp_localize_script(
				...
				array(
					...
					'affirmInlineEndpointNonce'  =>
						wp_create_nonce( 'wc_affirm_inline_checkout' ),
	...
	public function ajax_inline_checkout() {
		check_ajax_referer( 'wc_affirm_inline_checkout', 'affirm_checkout_nonce' );

		$bill_first_name = isset( $_POST['billing_first_name'] ) ? wc_clean( wp_unslash( $_POST['billing_first_name'] ) ) : '';
		$bill_last_name  = isset( $_POST['billing_last_name'] ) ? wc_clean( wp_unslash( $_POST['billing_last_name'] ) ) : '';
		$ship_first_name = isset( $_POST['shipping_first_name'] ) ? wc_clean( wp_unslash( $_POST['shipping_first_name'] ) ) : $bill_first_name;
		$ship_last_name  = isset( $_POST['shipping_last_name'] ) ? wc_clean( wp_unslash( $_POST['shipping_last_name'] ) ) : $bill_last_name;
		$phone           = isset( $_POST['billing_phone'] ) ? wc_clean( wp_unslash( $_POST['billing_phone'] ) ) : '';
		$email           = isset( $_POST['billing_email'] ) ? wc_clean( wp_unslash( $_POST['billing_email'] ) ) : '';
		$name_array = array(
	...
	public function admin_enqueue_scripts_order( $hook ) {
		...
		if ( array_key_exists('id', $_REQUEST ) ) {
			$order_id = absint( wc_clean( wp_unslash( $_REQUEST['id'] ) ) );
		}
	...
	public function api_key_check() {
		if ($this->get_gateway()->validate_api_keys_active()) {
			update_option('affirm_us_keys_active', true);
